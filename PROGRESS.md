...
**Features:**
- ✅ Auto-update check on startup (not on PLAY click)
- ✅ Launcher self-update detection
- ✅ Shows "X updates available" dialog
- ✅ User can choose to update or skip
- ✅ Progress bar with smooth 0-100% progression
- ✅ Status messages at every stage:
  - "Connecting to patch server..."
  - "✓ Connected - Checking files..."
  - "📥 Downloading filename (1/5)"
  - "✓ All files updated - Ready to play"
  - "🎮 Launching EverQuest..."
- ✅ Graphics Settings menu with resolution detection
...
├── patcher-config.json    ← Pre-configured
├── eq-patcher-client.zip  ← All-in-one bundle
├── spells_us.txt          ← Your custom files
├── UI/
│   └── default/
│       └── EQUI_*.xml
...
```
C:\EverQuest\
├── eqgame.exe             ← Game executable
├── LaunchPad.exe          ← Your launcher
├── patcher.exe            ← CLI fallback
├── patcher-config.json    ← Server config
├── eqclient.ini           ← Graphics settings
└── [patched files]        ← Downloaded from server
```
...
### Fixed Issues
1. ✅ IPv6 detection → Now forces IPv4
2. ✅ Wrong nginx paths → Fixed to /var/www/html/eq-patches
3. ✅ Missing mingw-w64 → Now uses pre-compiled executables
4. ✅ Icon not showing → Fixed with windres
5. ✅ Root user rejected → Now works as root or regular user
6. ✅ Website button not showing → Fixed URL check
7. ✅ Resolution list too generic → Now queries actual display modes
...
### Won't Implement
- ❌ Complex YAML configs (keeping it simple)
- ❌ Multiple client types (one launcher for all)
- ❌ Automatic game launching after patch (user choice)
